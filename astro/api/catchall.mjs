...
// 2026-08-11 v2 marker (Railway Ê£ÑÁî®, ?πÁî® Vercel edge function; force rebuild)
//
...
// 2026-08-26: ?πÊ? IP ?ΩÂ???+ ?ØÈÅ∏ÂØÜÁ¢ºÔºà‰ªª‰∏Ä?öÈ??≥ÂèØÔº?//   - STOCK_OPERATOR_IPS=1.2.3.4,5.6.7.8,192.168.0.0/16  ??‰Ω†Á? IP
//   - STOCK_OPERATOR_PASSWORD=xxx                       ??ÂæûÂ??¢Ë®™?èÁî®
//   ?©ËÄÖÈÉΩÊ≤íË®≠ ??ÂÆåÂÖ®?íÁ?Ôºàfail-closedÔºâ„Ä?function operatorOk(provided, request) {
  // IP ?ΩÂ???  const ipList = pickStr(process.env.STOCK_OPERATOR_IPS);
  if (ipList && request) {
...
function _clientIp(request) {
  // Vercel edge sets x-forwarded-for (?óË??ÜÈ?ÔºåÁ¨¨‰∏Ä?ãÊòØ client IP)
  // x-real-ip ??fallback
...
function _ipInCidr(ip, cidr) {
  ...
  // IPv4 only (Vercel edge Â∞?IPv6 ?®‰???headerÔºåÈÄôË£°Á∞°Â??™ÊîØ??IPv4)
...
// ?Ä?Ä helpers ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä watchlist + candle helpers ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä handlers: health & basic CRUD ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
async function listStocks(request) {
  try {
    ...
    return json({ ok: true, source: "seed", count: 4, stocks: [
      { code: "2330", name: "?∞Á???, ticker: "2330.TW" },
      { code: "2454", name: "?ØÁôºÁß?, ticker: "2454.TW" },
      { code: "2317", name: "È¥ªÊµ∑",   ticker: "2317.TW" },
      { code: "0050", name: "?ÉÂ§ß?∞ÁÅ£50", ticker: "0050.TW" },
    ], db_error: e?.message });
  }
}
...
async function addStock(request) {
  ...
  if (!operatorOk(body?.password, request)) return json({ error: "ÂØÜÁ¢º?ØË™§" }, { status: 403 });
  const code = pickStr(body?.code).trim();
  if (!/^\d{4,6}$/.test(code)) return json({ error: "Áº∫Â??ñÁÑ°?àÁ?‰ª??" }, { status: 400 });
...
async function removeStock(request, code) {
  ...
  const body = await readJson(request);
  if (!operatorOk(body?.password, request)) return json({ error: "ÂØÜÁ¢º?ØË™§" }, { status: 403 });
  if (!code) return json({ error: "Áº∫Â?‰ª??" }, { status: 400 });
  try {
...
async function stockKlines(request, ticker) {
  ...
  try {
    ...
    // 2026-08-26: ?πÊ? 200 + Á©?candlesÔºåÈÅø?çÂ?Á´?lightweight-charts ??"Value is null"??    //   ?ôÂÄ?endpoint ?ØÂÖ¨??cacheable lookupsÔºåÂ? 404 ?ÉË??çÁ´Ø fetch ?≤ÂÖ• catch ?ÜÊîØÔº?    //   ‰ΩÜË??àÂ?Á´Ø‰??ïÁ? catch ?¥Êé• setData([])ÔºåÊñ∞?àÂ???try/catch ‰Ω?console ?ÑÊòØ?ÉÂà∑?ØË™§??    //   ?πÊ? 200 + empty ÁµêÊ?ÔºåÂ?Á´ØÂèØ‰ª•Ê≠£Â∏?render ?åÊ≠§?°Á•®??K Á∑öË??ô„ÄçË??Ø„Ä?    if (!rows.length) {
      return json({
        ...
        message: "Ê≠§ËÇ°Á•®Â???K Á∑öË??ôÔ??ØËÉΩÂ∞öÊú™ seed ??market_price_barsÔº?,
      });
    }
    ...
    // 2026-08-14: Ë£?capital/financial/income Áµ¶Âè≥??panelÔºàËÇ°??Â∏ÇÂÄ?ÊØèËÇ°Ê∑®ÂÄ?EPS/?¨Á?ÊØ?ROE/ROA/ÊØõÂà©???üÁ???Ê∑®Âà©??ÊÆñÂà©?áÔ?
    // Âæ?financial_reports ?âÊ??∞‰?Á≠?(symbol, period, revenue, gross_profit, operating_income, net_income, eps)
    let finLatest = null;
    try {
      const fr = await q(
    ...
    // Âæ?market_instruments.metadata_text ??shares_outstanding (?ÉÂ§ß/ÂØåÊ?Á≠â‰?Ê∫êÊ???
    ...
    const marketCap = sharesOutstanding && last.close ? Math.round(sharesOutstanding * last.close / 1e8) : null;  // ??    return json({
      ...
      // 2026-08-14: ?≥ÂÅ¥ panel ?®Á?Ë≥áÊú¨/Ë≤°Â?/‰º∞ÂÄºÊ?‰ΩçÔ?Âæ?financial_reports + market_instruments.metadata_text ?âÔ?
      capital: {
...
// (real TAIEX data, free, no key needed). The frontend page renders this in TWII/Â§ßÁõ§ mode.
//
// 2026-08-13: ?∞Â? summary / gaps / dipSignal / markers Ê¨Ñ‰?ÔºàÂ?Á´?loadIndexChart ?üÂ???4 ?ãÔ?
//   - summary: Â§ßÁõ§?Ä?ãÈù¢?øÔ?openGapUp/Down, bias, nearestSupport/ResistÔº?//   - gaps: Áº∫Âè£Ê∏ÖÂñÆ?¢ÊùøÔºàtype, gap_bottom/top, gap_pct, filled, fill_dateÔº?//   - dipSignal: ?ÑÂ?Ë®äË??¢ÊùøÔºàtriggered, drop_pct, is_vol_max, has_bearish_gapÔº?//   - markers: ?ãËÇ°‰∫§Ê?Ë®äË? markers ‰∏çÈÅ©?®Êñº?áÊï∏ÔºàË??ôÊòØ 2330 proxyÔºâÔ?‰øùÊ?Á©?array
//
// 2026-08-13: ^TWII ?πÁî® Yahoo Finance ^TWII ?üÂØ¶?áÊï∏ÔºàÂ?‰ª?2330 proxyÔºâ„ÄÇYahoo 5d/min rate limit
//   ‰Ω?Vercel edge IP ???Ôºåuser ?è‰?‰∏çÊ??û„ÄÇÂ§±??fallback 2330??
// Helper: ??Yahoo Finance ?áÊï∏/?ãËÇ°??KÔºåÂ???row-shaped ???
// ÂΩ¢Á?Â∞çÈ? Neon `market_price_bars` ?•Ë©¢ÁµêÊ?ÔºöÊ??ãÂ?Á¥?{trade_date: "YYYY-MM-DD", open_price, high_price, low_price, close_price, volume, change_value}
...
async function indexKlines(request, ticker) {
  // 2026-08-13: Vercel Ë∑ØÁî± regex Ê≤?decodeURLÔºåticker ?øÂà∞?ÑÊòØ "%5ETWII" ?å‰???"^TWII"
  //             ??decode ?çÊ?Â∞?  const decodedTicker = (() => { try { return decodeURIComponent(ticker); } catch { return ticker; } })();
  const isTwii = decodedTicker === "^TWII";
  const proxy = "2330";  // fallback proxy (TSMC ?∂Â§ß?§Ë?‰º?
  let actualSource = "db";
  try {
    const u = urlOf(request);
    ...
    if (!rows.length) return json({ error: "?•ÁÑ°Ë≥áÊ? (proxy " + proxy + ")", ticker }, { status: 404 });
    // 2026-08-13: Yahoo Ë≥áÊ???ascending (oldest first)ÔºåDB Ë≥áÊ???descending (newest first)
    //             ??actualSource ?§Êñ∑Ë¶Å‰?Ë¶ÅÂ? reverse
    ...
    // ??Áº∫Âè£?µÊ∏¨ÔºàË? computeGapsForSymbol ?±Áî®?èËºØÔºå‰???inline candles ‰∏çÈ???DBÔº?    const gapStart = Math.max(1, candles.length - gapLookback);
    const gaps = [];
    ...
    // ???ÑÂ?Ë®äË?Ôºàdip signalÔºâÔ?3 Ê¢ù‰ª∂ = ?™Â?Ë£úÂ?‰∏ãÁº∫??+ Ëø?7 ?•Ë?Âπ???8% + ‰ªäÊó•?ê‰∫§?èÁÇ∫Ëø?7 ?•Ê?Â§?    // Ê≥®Ê?Ôºöcandles ??ascendingÔºàË??íÊñ∞ÔºâÔ??Ä‰ª•Ê?Âæå‰??πÊòØ‰ªäÂ§©ÔºåÂÄíÊï∏Á¨?1 ?πÊòØ?®Â§©
    const dipLookback = 7;
    const dipWindowAll  = candles.slice(-Math.min(dipLookback, candles.length));  // ?ÄËø?N ?•Âê´‰ªäÊó•
    const dipWindowPrev = candles.slice(-(Math.min(dipLookback, candles.length) + 1), -1);  // ?ÄËø?N ?•‰??´‰???    const firstClose7 = dipWindowAll.length ? dipWindowAll[0].close : last.close;
    ...
    // drop_pct ?Ø„ÄåÊ≠£?º„ÄçË°®Á§∫‰?Ë∑åÔ??çÁ´Ø UI È°ØÁ§∫??"-X%"Ôº?    const drop_pct = firstClose7 > 0 ? r2(((firstClose7 - last.close) / firstClose7) * 100) : 0;
    ...
    return json({
      ...
      latest: {
        code: decodedTicker,
        name: decodedTicker === "^TWII" ? "?†Ê??áÊï∏" : (decodedTicker === "^TWOII" ? "Ê´ÉË≤∑?áÊï∏" : null),
        close: last.close,
        change: r2(last.close - prev.close),
        changePct: prev.close ? r2(((last.close - prev.close) / prev.close) * 100) : 0,
        change_pct: prev.close ? r2(((last.close - prev.close) / prev.close) * 100) : 0,
      ...
      // ??2026-08-13: ?∞Â??õÂÄãÊ?‰Ω?      markers: [],          // ?ãËÇ°‰∫§Ê?Ë®äË? markersÔºàbuy/sell signalsÔºâ‰??©Áî®?ºÊ???proxy Ë≥áÊ?Ôºå‰??ÅÁ©∫ array
      summary,              // Â§ßÁõ§?Ä?ãÈù¢??      gaps,                 // Áº∫Âè£Ê∏ÖÂñÆ?¢Êùø
      dipSignal,            // ?ÑÂ?Ë®äË??¢Êùø
    });
  } catch (e) {
    return json({ error: e?.message }, { status: 500 });
  }
...
// ?Ä?Ä screener core: score one stock ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
async function stockIntro(request, code) {
  ...
  try {
    ...
    // 2026-08-14: Ë£?capital (?°Êú¨/?°Êï∏) + marketCap (Â∏ÇÂÄ? Áµ¶Â?Á´ØÂü∫?¨Ë??ôÂ?Â°?    let capital = null;       // ?°Êï∏ÔºàsharesÔº?    let marketCap = null;     // Â∏ÇÂÄºÔ??ÉÔ?
    let lastClose = null;
    try {
    ...
    // 2026-08-14: Ë£?financial + valuation Áµ¶Â?Á´ØÂè≥??panelÔºàÂ? financial_reports ?âÔ?
    let finLatest = null;
    try {
      const fr = await q(
        `SELECT period, revenue, gross_profit, operating_income, net_income, eps
    ...
    return json({
      ...
      marketCap,
      lastClose,
      // 2026-08-14: ?∫Â?Á´ØÂè≥??panel ?ê‰??åÂü∫?¨Ê?Ê≥Å„Ä??åË≤°?ôË?Ë®ä„ÄçÁî®?ÑÊ?Âπ≥Ê?‰Ω?      eps: epsVal,
      pe: peRatio,
...
async function warmingZoneScan(request) {
  ...
  const items = results
    ...
    .map((r) => ({ ...r, category: r.score >= 4 ? "Âº∑Âã¢" : "ËΩâÂº∑" }));
  return json({
...
async function addRecipient(request) {
  ...
  if (!name || !email) return json({ error: "Áº∫Â? name ??email" }, { status: 400 });
  try {
...
async function removeRecipient(request) {
  ...
  if (!id) return json({ error: "Áº∫Â? id" }, { status: 400 });
  try {
...
async function macroData(request) {
  // macroData: for macro.html. Frontend wants `data.data = [{?áÊ?, ?Ä?∞ÂÄ? ?çÂÄ? ?¥Êñ∞?ÇÈ?, ...}]`
  ...
  try {
    ...
    // ??‰øÆÊ≠£ BUG-7ÔºöÊ??å‰?Ê∫ê„ÄçÊ?‰ΩçÂ°´??"FRED" ?çËÉΩËÆ?macro.html ??renderFredTable È°ØÁ§∫?∫‰???    //   ‰πãÂ???"macro_yields" / "TSMC proxy" / "?Ä Railway backend (offline)"Ôº?    //   renderFredTable ?™È?Êø?d["‰æÜÊ?"] === "FRED" ??‰∏ÄÂæãË¢´?éÊøæ????È°ØÁ§∫?åÁÑ°?∏Ê??ç„Ä?    //   ?πÊ?ÔºöÊ???macro yield / Á∏ΩÁ??áÊ?Áµ±‰?Ê®ôË???"FRED"ÔºàÂØ¶?õÊï∏?ö‰?Ê∫êÔ?
    //   ??macro_yields Ë≥áÊ?Ë°?+ offline placeholderÔºâÔ??çÁ´ØÂ∞±Ê? render??    const arr = [
      { "?áÊ?": "?∞ËÇ°?áÊï∏‰ª?? (2330)", "?Ä?∞ÂÄ?: tsLast, "?çÂÄ?: tsPrev, "?¥Êñ∞?ÇÈ?": asOfLabel, "‰æÜÊ?": "FRED", "‰æÜÊ?Ê®ôË?": "TSMC proxy" },
      { "?áÊ?": "Áæ?0Âπ¥ÂÖ¨?µÊ??©Á?(%)", "?Ä?∞ÂÄ?: last10y, "?çÂÄ?: prev10y, "?¥Êñ∞?ÇÈ?": asOfLabel, "‰æÜÊ?": "FRED", "‰æÜÊ?Ê®ôË?": "macro_yields" },
      { "?áÊ?": "Áæ?Âπ¥ÂÖ¨?µÊ??©Á?(%)",  "?Ä?∞ÂÄ?: last2y,  "?çÂÄ?: prev2y,  "?¥Êñ∞?ÇÈ?": asOfLabel, "‰æÜÊ?": "FRED", "‰æÜÊ?Ê®ôË?": "macro_yields" },
    ];
    // Spread placeholder rows so the page can render placeholders for missing metrics.
    const placeholders = [
      "ÁæéÂÇµÂπ≥Ë°°?öËÜ®?áBEI(%)", "10Âπ?3?àÂÖ¨?µÂà©Â∑?, "10Âπ?2Âπ¥ÂÖ¨?µÂà©Â∑?,
      "?ØÈÇ¶?∫È??©Á?(%)", "GDP?êÈï∑?áÂπ¥??%)", "ÁæéÂ?Â§±Ê•≠??%)", "ÂØÜÂ§ßÊ∂àË≤ª?Ö‰ø°Âø?,
      "Â∏≠Â??¨Á?ÊØ?CAPE)", "VIX?êÊ??áÊï∏", "ÁæéÂ?CPIÂπ¥Â???%)", "?∏Â?CPI YoY(%)",
    ];
    for (const name of placeholders) {
      arr.push({ "?áÊ?": name, "?Ä?∞ÂÄ?: null, "?çÂÄ?: null, "?¥Êñ∞?ÇÈ?": asOfLabel, "‰æÜÊ?": "FRED", "‰æÜÊ?Ê®ôË?": "?Ä Railway backend (offline)" });
    }
    return json({
      ok: true,
      source: "db",
...
async function markersRecordImpl(request) {
  ...
  // ?©Á®ÆÂØ´ÂÖ•Ê®°Â?:
  //   A) ÁÆ°Á??°Ê???body Â∏?password + ?ÆÁ? (date/type/text/price)    ???ÄË¶ÅÂ?Á¢?  //   B) ?çÁ´Ø?™Â?Ë®òÈ?:body.items[] ???                                ???çÂ?Á¢??¨È??ÅÈù¢‰∫ã‰ª∂Ë®òÈ?)
  const items = Array.isArray(body?.items) ? body.items : null;
  const hasPassword = operatorOk(body?.password, request);

  // Ê®°Â? A:ÁÆ°Á??°Ê??ïË????ëÂ??∏ÂÆπ)
  ...
  // Ê®°Â? B:?çÁ´Ø?™Â?Ë®òÈ? markers(?çÂ?Á¢?
  if (items && items.length > 0) {
    try {
      ...
      for (const it of items) {
        ...
        // 2026-08-26 fix: ?™Â?Á¥îÊ?Â≠óÂà∞ markers.text Ê¨Ñ‰?
        //   ‰πãÂ???close/ma5/10/20/60/position/shape/color/time Â∫èÂ??ñÂ???textÔº?        //   ?†Ê? marker_history.html È°ØÁ§∫ "<Ë®äÊÅØ> || {...‰∏Ä??null JSON...}" ?úË???        //   ?ô‰?Ê¨Ñ‰???charts Á´ØÂ∑≤Á∂ìÁî®ÂæóÂà∞Ôºå‰??ÄË¶ÅÂ??ûÁ???DB ??text??        const text = pickStr(it?.text);
...
// ?Ä?Ä new handlers: market_gaps, fibonacci, scan_and_email ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
async function computeGapsForSymbol(symbol, label, lookback, minGap) {
  try {
    ...
    if (!rows.length) return { name: label, error: "?•ÁÑ°Ë≥áÊ?" };
    const asc = rows.slice().reverse();
    const gaps = [];
    ...
    return { name: label, error: e?.message || "?•Ë©¢Â§±Ê?" };
  }
}
...
async function marketGaps(request) {
  ...
  // 2330 (?∞Á??? so the panel can show something live.
  const [twii, twoii, proxy] = await Promise.all([
    Promise.resolve({ name: "?†Ê??áÊï∏", error: "?°Ê??∏Ë??ôÔ?DB ?ÖÂ??ãËÇ°/ETFÔº? }),
    Promise.resolve({ name: "Ê´ÉË≤∑?áÊï∏", error: "?°Ê??∏Ë??ôÔ?DB ?ÖÂ??ãËÇ°/ETFÔº? }),
    computeGapsForSymbol("2330", "?∞Á???(2330) ?∂‰?Â§ßÁõ§‰ª??", lookback, minGap),
  ]);
...
async function fibonacciFor(request, code) {
  ...
  try {
    ...
    if (candles.length < 20) return json({ error: "Ë≥áÊ?‰∏çË∂≥", code, window }, { status: 404 });
...
// ?Ä?Ä new handlers: etf_holdings/* ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
  ...
  try {
    ...
    if (holdingsCount === 0) {
      return json({
        ...
        count: 0,
        no_data: true,
        message: "etf_holdings Ë°®ÁÇ∫Á©∫Ô???bulk data sourceÔºàÈ? per-issuer ?¨Ô?",
      });
    }
  ...
  try {
    ...
    if (n === 0) {
      return json({
        ...
        no_data: true,
        count: 0,
        message: "etf_holdings Ë°®ÁÇ∫Á©∫Ô???bulk sourceÔºõÈ? per-issuer ?¨Ê??ãÂ? seedÔº?,
      });
    }
...
// ?Ä?Ä new handlers: rebalance/* ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä new handlers: uptrend_watch/* ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
async function uptrendWatch(request) {
  // ??‰øÆÊ≠£ BUG-8Ôºöuptrend-watch.html ?çÁ´Ø?üÂ?ÁµêÊ???  //   { ok, as_of, scanned, uptrend_count, ma10:[...], ma20:[...], volow:[...] }
  //   ?äÁ??™Â???itemsÔºàÊ∑∑?úÁ?ÁØ©ÈÅ∏ÁµêÊ?ÔºâÔ?Â∞éËá¥?çÁ´Ø?ÉÊ?Á∏ΩÊï∏/Ë∂®Âã¢Ê™îÊï∏/?ûË∏©/?ÜÈ?‰∏ãÊÆ∫ ?®ÊòØ 0Ôº?  //   ‰∏âÂÄ?tab ‰πüË?‰∏çÂà∞Ë≥áÊ???  //   ?πÁÇ∫ÔºöÁî® scanAllImpl ÁÆóÂá∫‰∏âÈ?Ê∏ÖÂñÆÔºå‰∏¶?âÂ?Á´ØÊ?‰ΩçÂ??≥„Ä?  try {
    const results = await scanAllImpl();
    ...
    const scored = (arr) => (arr || []).map((r) => ({
      ...
      score: r.score,
      status: r.score >= 4 ? "Âº∑Âã¢Â§öÈ†≠" : "ËΩâÂº∑ËßÄÂØ?,
    }));

    const uptrendAll = results.filter((r) => r.cond2 && r.cond3);
    // ?ûË∏©?áÁ? = ?ÆÂ??•Ë? MA10 ??MA20 ‰ΩÜ‰??®Â??≠Ê???    const ma10 = uptrendAll.filter((r) =>
    ...
    // ?ÜÈ?‰∏ãÊÆ∫ÔºàÁ?‰ººÈåØÊÆ∫Ô?= È´òÊ?‰∫§È? + ?∂Áõ§?†Èõ¢ MA20
    ...
    return json({
      ...
      items: [],
      error: e?.message,
      message: "uptrend_watch Ë®àÁ?Â§±Ê?ÔºàÂèØ?ΩÁº∫Â∞?watchlist / market_price_barsÔº?,
    });
  }
}
async function uptrendWatchFilter(request) { return uptrendWatch(request); }

// ?Ä?Ä new handlers: admin/logs (markers as log) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä new handlers: conference, exdiv, etc. ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// /api/conference/<code> ??per-stock conference / Ê≥ïË™™??lookup. Frontend
...
async function exdivCalendar(request) {
  ...
  try {
    ...
    // 2026-08-14: ??updated_at / as_of / fetched_at Áµ?exdiv.html È°ØÁ§∫?åÊõ¥?∞Ê??ì„Ä?    const nowIso = new Date().toISOString();
    return json({
...
// ?Ä?Ä institutional / foreign_futures / financial / revenue etc. ?Ä?Ä?Ä?Ä?Ä?Ä?Ä
  ...
  try {
    ...
    if (!rows.length) {
      return json({
        ...
        source: "stub",
        code,
        quality: { score: 0, level: "?°Ë???, reasons: [], warnings: ["Ê≠§ËÇ°Á•®Â??°Â≠£Â∫¶Ë≤°?±Ë???] },
        financial_data: { eps: [] },
        error: "Ê≠§ËÇ°Á•®Â??°Â≠£Â∫¶Ë≤°?±Ë???,
        message: "synth seed ?ÖÊ∂µ?ãÈÉ®??watchlistÔºåË?Ë¶ãÂ???,
      });
    }
    ...
    if (gm >= 0.4) { score += 15; reasons.push(`ÊØõÂà©??${(gm*100).toFixed(1)}% ?™Áï∞`); }
    else if (gm >= 0.2) { score += 8; reasons.push(`ÊØõÂà©??${(gm*100).toFixed(1)}% Á©©Â?`); }
    else if (gm < 0.1 && gm >= 0) { score -= 5; warnings.push(`ÊØõÂà©?áÂ? ${(gm*100).toFixed(1)}%`); }
    if (om >= 0.2) { score += 15; reasons.push(`?üÁ???${(om*100).toFixed(1)}% Âº∑Â?`); }
    else if (om >= 0.1) { score += 5; reasons.push(`?üÁ???${(om*100).toFixed(1)}% ?•Â∫∑`); }
    else if (om < 0) { score -= 10; warnings.push(`?üÊ•≠?ßÊ? ${(om*100).toFixed(1)}%`); }
    if (nm >= 0.15) { score += 10; reasons.push(`Ê∑®Âà©??${(nm*100).toFixed(1)}%`); }
    else if (nm < 0) { score -= 15; warnings.push(`Ê∑®Ê? ${(nm*100).toFixed(1)}%`); }
    if (periods.length >= 2) {
      const oldest = periods[periods.length - 1];
      const epsGrowth = oldest.eps !== 0 ? ((latest.eps - oldest.eps) / Math.abs(oldest.eps)) : 0;
      if (epsGrowth > 0.2) { score += 10; reasons.push(`EPS Â≠?? ${(epsGrowth*100).toFixed(1)}%`); }
      else if (epsGrowth < -0.2) { score -= 10; warnings.push(`EPS Â≠?? ${Math.abs(epsGrowth*100).toFixed(1)}%`); }
    }
    score = Math.max(0, Math.min(100, Math.round(score)));
    const level = score >= 80 ? "?™Áï∞" : score >= 60 ? "?ØÂ•Ω" : score >= 40 ? "‰∏≠Á?" : score >= 20 ? "ÂæÖË?ÂØ? : "È´òÈ¢®??;
    ...
    try {
      ...
      if (close > 0 && eps_ttm > 0) {
        ...
        try {
          ...
          if (m && m.industry === "?äÂ?È´îÊ•≠") pbr = 6;
          else if (m && m.industry === "?ëË?‰øùÈö™Ê•?) pbr = 1.0;
          else if (m && m.industry === "?™È?Ê•?) pbr = 1.5;
        } catch {}
    ...
    return json({
      ...
      combined: {
        ...
        confidence: reasons.length > warnings.length ? "È´? : "‰∏?,
        breakdown: {
...
async function marginBurst(request, codeFromPath) {
  ...
  // what we have, and "?çË?Ë≥áÊ?Â∞öÊú™Âª∫Á?" badge for the rest.
  if (codeFromPath) {
    ...
    return json({
      ...
      metrics: {
        code: codeFromPath,
        is_g7: false,
        fail_reasons: ["?çË?È§òÈ?Ë≥áÊ??™Âª∫Á´ãÔ?margin_balance Ë°®ÁÇ∫Á©∫Ô?TWSE ??per-stock ?¨È? sourceÔº?],
  ...
  try {
    ...
    const sql = `
      SELECT r.id, r.symbol AS code,
             -- ??‰øÆÊ≠£ BUG-5ÔºöÂÑ™?àÁî® watchlist ?çÁ®±ÔºàËá™?∏ËÇ°Â∑≤ÂëΩ?çÔ?Ôºåfallback ??             --   market_instruments.display_nameÔºà‰?Â∏ÇÊ??®Â??¥Â??ßË°®ÔºâÔ??ÄÂæåÊ??®Á©∫Â≠ó‰∏≤??             --   revenue Ë°®Ë??ÑÂÖ®Â∏ÇÂ†¥?°Á•®Ôºå‰? watchlist ?™Ê? 7 ?ØËá™?∏ËÇ°ÔºåÂ???join
             --   market_instruments ?çËÉΩËÆìÊ??âËÇ°Á•®Á??åÂ?Á®±„ÄçÊ?‰ΩçÊ??º„Ä?             COALESCE(NULLIF(w.name, ''), NULLIF(inst.display_name, ''), '') AS name,
      ...
      -- ????symbol ?ØËÉΩÂ∞çÊ?Â§öÂÄ?sourceÔºàyahoo/finmind Á≠âÔ?ÔºåÁî® DISTINCT ON ?ñ‰ªª‰∏ÄÁ≠ÜÊ? display_name ?ÑÂ?
      LEFT JOIN (
...
// ?Ä?Ä sold_too_early: heuristic "potentially sold too early" detector ?Ä?Ä
  ...
  try {
    ...
    if (wl.length === 0) {
      return json({ ok: true, source: "stub", as_of: new Date().toISOString().slice(0, 10),
        ...
        message: "watchlist ?∫Á©∫ÔºåË??àÂú®‰∏ªÁ≥ªÁµ±Êñ∞Â¢ûËá™?∏ËÇ°" });
    }
    ...
    return json({
      ...
      count: hits.length,
      rows: hits,
      note: "?üÁôºÂºèÔ??°ÂÉπ?æË???MA20ÔºàË≥£?∫Ë??üÔ?ÔºåÂ?‰æÜÂ?Á´ôÂ? MA5/10/20Ôºå‰??°ÂØ¶?õ‰∫§?ìÁ???,
    });
  } catch (e) {
  ...
  try {
    ...
    if (rows.length === 0) {
      return json({ ok: true, source: "stub", count: 0, items: [], as_of: new Date().toISOString().slice(0, 10),
        ...
        light: "gray", headline: "ai_capex Ë°®ÁÇ∫Á©∫Ô?Ë´ãË? SEC EDGAR loader" });
    }
    ...
    // Light signal
    let light = "gray";
    let headline = "Ë≥áÊ?‰∏çË∂≥";
    if (aggYoyPct != null) {
      if (aggYoyPct > 15 && (accelPp == null || accelPp >= 0)) { light = "green"; headline = `?àË? capex TTM ‰ªçÊì¥ÂºµÔ?YoY +${aggYoyPct}%Ôºâ`; }
      else if (aggYoyPct > 0) { light = "yellow"; headline = `?¥Âºµ‰ΩÜÂ??ΩË?Âº±Ô?YoY +${aggYoyPct}%${accelPp != null ? `Ôº?{accelPp > 0 ? '+' : ''}${accelPp}pp` : ''}Ôºâ`; }
      else { light = "red"; headline = `?àË? capex TTM ËΩâÊî∂Á∏ÆÔ?YoY ${aggYoyPct}%Ôºâ`; }
    }
    const asOf = new Date().toISOString().slice(0, 10);
    return json({
      ok: true,
...
// ?Ä?Ä price_compare / heatmap ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä etf_signal_filter / stock_damo_filter / stock_news_scan ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
async function stockDamoFilter(request) {
  // "Â§ßÊ?" filter: cond2+cond3+cond4 + above MA20 (similar to signal_filter but a different threshold view)
  const results = await scanAllImpl();
  const items = results.filter((r) => r.cond2 && r.cond3 && r.cond4).map((r) => ({ ...r, status: "Â§ßÊ??ôÈÅ∏" }));
  return json({ ok: true, source: "db", count: items.length, items, generated_at: Date.now() });
}
...
async function stockNewsScanQuota(request) {
  // Á∞°Ê? quota È°ØÁ§∫:‰ªäÂ§©Ë∑ëÈ?ÂπæÊ¨° (from markers)
  try {
    const { rows } = await q(
    ...
    return json({
      ...
      // aliases used by etf_holdings_tracker.html ("‰ªäÊó•?©È? X / Y")
      left: cap - used, cap,
    });
  } catch (e) {
...
// ?Ä?Ä etf_pivot/* real handlers ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
async function etfDiff(request, code) {
  ...
  try {
    ...
    if (dates.length === 0) {
      return json({ ok: true, source: "db", code, status: "no_diff", snapshot_count: 0, message: "Â∞öÁÑ°Âø´ÁÖßÔºåË??àÊ??åÁ??≥Ê?Âø´ÁÖß??, summary: { added: 0, removed: 0, changed: 0, unchanged: 0 }, holdings: [], new_time: null, old_time: null });
    }
    if (dates.length === 1) {
      return json({ ok: true, source: "db", code, status: "no_diff", snapshot_count: 1, message: "?™Ê? 1 Á≠ÜÂø´?ßÔ??°Ê?Ë®àÁ?Â∑ÆÁï∞ÔºàÈ? ??2 Á≠ÜÔ?", summary: { added: 0, removed: 0, changed: 0, unchanged: 0 }, holdings: [], new_time: String(dates[0]).slice(0, 10), old_time: null });
    }
...
// ?Ä?Ä single-resource GETs (markers/<id>, recipients/<id>, etc.) ?Ä?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä data loaders (fetch from TWSE public API ??insert into Neon) ?Ä?Ä?Ä?Ä
function twseDateStr(d) {
  // input: Date object ??"YYYYMMDD" (Ê∞ëÂ?Âπ¥Ëá™?ïË?Ë•øÂ?)
  return d.getFullYear().toString() +
    String(d.getMonth() + 1).padStart(2, "0") +
    String(d.getDate()).padStart(2, "0");
}
function rocToIsoDate(rocStr) {
  // "115Âπ?7??1?? ??"2026-07-31"
  const m = /(\d+)Âπ?\d+)??\d+)??.exec(rocStr);
  if (!m) return null;
  return `${parseInt(m[1], 10) + 1911}-${m[2]}-${m[3]}`;
}
...
async function loadInstitutionalForDate(dateYmd) {
  ...
  // TWSE T86 Ê¨Ñ‰??ÜÂ?(2026 Á¢∫Ë?):
  // 0: Ë≠âÂà∏‰ª??, 1: Ë≠âÂà∏?çÁ®±
  // 2-4: Â§ñÈô∏Ë≥áË≤∑??Ë≥?á∫/Ë≤∑Ë≥£Ë∂?‰∏çÂê´Â§ñË??™Á???
  // 5-7: Â§ñË??™Á??ÜË≤∑??Ë≥?á∫/Ë≤∑Ë≥£Ë∂?  // 8-10: ?ï‰ø°Ë≤∑ÈÄ?Ë≥?á∫/Ë≤∑Ë≥£Ë∂?  // 11: ?™Á??ÜË≤∑Ë≥??(Á∏?
  // 12-14: ?™Á????™Ë?)Ë≤∑ÈÄ?Ë≥?á∫/Ë≤∑Ë≥£Ë∂?  // 15-17: ?™Á????øÈö™)Ë≤∑ÈÄ?Ë≥?á∫/Ë≤∑Ë≥£Ë∂?  // 18: ‰∏âÂ§ßÊ≥ï‰∫∫Ë≤∑Ë≥£Ë∂?  const symbols = [];
  ...
  for (const r of data.data) {
    ...
    if (!/^\d{4,6}$/.test(sym)) continue; // ?™Êî∂Á¥îËÇ°Á•®‰ª£??    symbols.push(sym);
    foreignBuy.push(numFromStr(r[2]) + numFromStr(r[5]));   // Â§ñÈô∏Ë≥?+ Â§ñË??™Á?
    ...
    dealerNet.push(numFromStr(r[11])); // Â∑≤ÊòØÁ∏ΩË≤∑Ë≥??
  }
...
// ?Ä?Ä overseas indices loader (Yahoo Finance unofficial chart API) ?Ä?Ä
...
async function loadOverseasIndices(request) {
  ...
  if (request.method === "POST" && !operatorOk(body?.password, request)) {
    return json({ error: "ÂØÜÁ¢º?ØË™§" }, { status: 403 });
  }
...
// ?Ä?Ä loadMacroYields: Yahoo Finance US Treasury yields ??macro_yields ?Ä
// Series mapping:
...
// ?Ä?Ä loadMacroNews: Google News RSS ??knowledge_library (record_type=news) ?Ä
async function loadMacroNews(request) {
  ...
  // Default queries: ?∞ËÇ° + ?ãÈ?
  const queries = (body?.queries && body.queries.length) ? body.queries
    : ["?∞ËÇ° ?†Ê??áÊï∏", "?∞Á???2330", "ÁæéËÇ° Ê®ôÊôÆ", "ÁæéËÅØÊ∫ñÊ? ?©Á?", "?∞ËÇ° Ê≥ï‰∫∫"];
...
// ?Ä?Ä loadIndexInstitutional: TWSE Â§ßÁõ§ ‰∏âÂ§ßÊ≥ï‰∫∫ ??index_institutional ?Ä
// Endpoint: https://www.twse.com.tw/fund/BFI82U?response=json&dayDate=YYYYMMDD
// fields: ["?Æ‰??çÁ®±","Ë≤∑ÈÄ≤È?È°?,"Ë≥?á∫?ëÈ?","Ë≤∑Ë≥£Â∑ÆÈ?"]
// rows:   [?™Á????™Ë?Ë≤∑Ë≥£), ?™Á????øÈö™), ?ï‰ø°, Â§ñË??äÈô∏Ë≥? Â§ñË??™Á??? ?àË?]
async function loadIndexInstitutionalForDate(dateYmd, indexCode) {
  ...
  // Aggregate by Ê≥ï‰∫∫ type:
  //   foreign_net = row 3 (Â§ñË?) + row 4 (Â§ñË??™Á???
  //   trust_net   = row 2 (?ï‰ø°)
  //   dealer_net  = row 0 (?™Á??ÜËá™Ë°? + row 1 (?™Á??ÜÈÅø??
  const parseNum = (s) => Number(String(s || "0").replace(/,/g, "").trim()) || 0;
  let foreign_net = 0, trust_net = 0, dealer_net = 0;
  for (const r of data.data) {
    const name = String(r[0] || "").trim();
    const diff = parseNum(r[3]); // Ë≤∑Ë≥£Â∑ÆÈ?
    if (name.startsWith("Â§ñË?")) foreign_net += diff;
    else if (name.startsWith("?ï‰ø°")) trust_net = diff;
    else if (name.startsWith("?™Á???)) dealer_net += diff;
  }
  await q(
...
// ?Ä?Ä loadMarkers: Âæ?screener + market_price_bars ?™Â?ÂØ´ÂÖ• markers (Ë≤∑Ë≥£Ë®äË?) ?Ä
// Ë¶èÂ?ÔºàÁ∞°?ÆÁ?ÔºåÂ? seed ?∫Ê??ßÂÆπ?ÑË??ôÔ?:
//   1. Á´ô‰?‰∏âÂ?Á∑?+ ?èÂ? ??'buy_chase'
//   2. Ë∑åÁ†¥ MA20 ??'sell_stop'
//   3. Â§ñË???≤∑ 3 ????'foreign_buy_3d'
//   4. Êº≤Â? (>= 9.5%) ??'limit_up'
async function loadMarkers(request) {
  ...
  try {
    ...
    for (const r of results) {
      const markers = [];
      // buy_chase: cond2 + cond3 + cond4 (Á´ô‰?‰∏âÂ?Á∑?+ 5d/20d Êº?
      if (r.cond2 && r.cond3 && r.cond4) {
        markers.push({
          code: r.code, date: today, type: "buy_chase",
          text: `Á´ô‰?‰∏âÂ?Á∑?+ 5??20?•Êº≤Âπ?${r.gain_5d_pct}%/${r.gain_20d_pct}%`,
      ...
      if (r.gain_5d_pct >= 9.5) {
        markers.push({
          ...
          text: `5?•Á¥ØË®àÊº≤Âπ?${r.gain_5d_pct}%ÔºàÁ?‰ººÊº≤????ùøÔºâ`,
          price: r.latest_close,
        });
      }
      // sell_stop: cond1 broken (Ë∑?60 ?•È? > 5%) + close < MA20
      const belowMA20 = r.latest_close < r.ma20;
      if (r.dist_high_60d_pct > 5 && belowMA20) {
        markers.push({
          code: r.code, date: today, type: "sell_stop",
          text: `Ë∑?60 ?•È? ${r.dist_high_60d_pct}%, Ë∑åÁ†¥ MA20 (${r.ma20})`,
          price: r.latest_close,
        });
      }
...
// ?Ä?Ä FinMind loaders (big_holders + financial_reports) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
// ?Ä $env:FINMIND_TOKEN, ??token ??fallback synth_v2_60 (Ë∑ëÁèæ??seed script)
...
// 2026-08-14: ?¨È? dataset ‰∏çÈ?Ë¶?tokenÔºàTaiwanStockShareholding / TaiwanStockInfo / TaiwanStockPrice Á≠âÔ?
// 600 req/hr ?êÈÄüÂ? 130 stocks ‰ªçË∂≥Â§?async function finmindFetchPublic(dataset, params = {}) {
  const u = new URL(FINMIND_BASE);
  u.searchParams.set("dataset", dataset);
  for (const [k, v] of Object.entries(params)) {
    if (v != null) u.searchParams.set(k, String(v));
...
// ?ì‰???stock ??big_holders (Ëø?1 Âπ¥Ê??àÊè≠??
...
async function loadBigHoldersFinMind(request) {
  ...
  if (!process.env.FINMIND_TOKEN) {
    return json({
      ok: false,
      source: "finmind",
      error: "FINMIND_TOKEN ?™Ë®≠ÂÆ???Ë´ãÂà∞ https://finmindtrade.com/ Ë®ªÂ?‰∏¶Ë®≠ÂÆ?Vercel env var",
      fallback: "?ÆÂ?‰ΩøÁî® synth_v2_60 synth Ë≥áÊ? (??seed-bh-60.mjs)",
    }, { status: 503 });
  }
...
// ?ì‰???stock ??financial_reports (Ëø?2 Âπ?quarterly)
...
async function loadFinancialReportsFinMind(request) {
  ...
  if (!process.env.FINMIND_TOKEN) {
    return json({
      ...
      source: "finmind",
      error: "FINMIND_TOKEN ?™Ë®≠ÂÆ???Ë´ãÂà∞ https://finmindtrade.com/ Ë®ªÂ?‰∏¶Ë®≠ÂÆ?Vercel env var",
      fallback: "?ÆÂ?‰ΩøÁî® synth_v2_60 synth Ë≥áÊ? (??seed-fr-60.mjs)",
    }, { status: 503 });
  }
  const wl = await q(`SELECT code FROM watchlist ORDER BY sort_order LIMIT 60`);
  const codes = wl.rows.map((r) => r.code);
...
// 2026-08-14: Âæ?FinMind TaiwanStockShareholding ??NumberOfSharesIssued (Â∑≤ÁôºË°åÊôÆ?öËÇ°??
// ÂØ´ÂÖ• market_instruments.metadata_text.shares_outstanding Áµ?stockIntro/stockKlines ÁÆ?marketCap
// ?¨È? dataset ‰∏çÁî® tokenÔºõÈ???600 req/hr
async function loadIssuedSharesFinMind(request) {
  ...
  for (let i = 0; i < codes.length; i += CONCURRENCY) {
    ...
    const chunkResults = await Promise.all(chunk.map(async (code) => {
      try {
        // ?ìÊ?Ëø?90 Â§©Ô?shareholding ÊØèÊ??¥Êñ∞Ôºå‰??∫‰??™Ëµ∑Ë¶ãÂ??ÄËøë‰?Á≠ÜÔ?
        const today = new Date();
...
async function loadExdivForDate(dateYmd) {
  ...
  // Field order: Ë≥áÊ??•Ê?, ?°Á•®‰ª??, ?°Á•®?çÁ®±, ?§Ê??ØÂ??∂Áõ§?? ?§Ê??ØÂ??ÉÂÉπ, Ê¨äÂÄ??ØÂÄ? Ê¨??? ...
  ...
  for (const r of data.data) {
    ...
    if (!ex_date) continue;
    const kind = String(r[6] || ""); // ??/ Ê¨?/ Ê¨äÊÅØ
    symbols.push(sym);
    ex_dates.push(ex_date);
    cash.push(kind.includes("??) ? numFromStr(r[5]) : 0);
    stock.push(kind.includes("Ê¨?) ? numFromStr(r[5]) : 0);
    types.push(kind);
  }
...
async function loadInstitutional(request) {
  const u = urlOf(request);
  const body = request.method !== "GET" ? await readJson(request) : {};
  // Vercel Cron ‰∏çÈÄ?body,‰πü‰?Â∏?password ???•Â? GET (?¨È? cron trigger)
  // ?ãÂ? trigger ‰ªçÂèØ POST + Â∏?password
  if (request.method === "POST" && !operatorOk(body?.password, request)) {
    return json({ error: "ÂØÜÁ¢º?ØË™§" }, { status: 403 });
  }
...
// ?Ä?Ä loadMarketPrices: TWSE STOCK_DAY_ALL ??market_price_bars ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
async function loadMarketPrices(request) {
  ...
  try {
    ...
    if (targets.size === 0) {
      return json({ ok: true, source: "stub", count: 0, message: "watchlist + etf_watchlist ?∫Á©∫" });
    }
    ...
    if (lines.length < 2) {
      return json({ ok: false, source: "loader", error: "TWSE ?ûÂÇ≥Á©∫Ë??ôÔ??ØËÉΩ?û‰∫§?ìÊó•Ôº? });
    }
    ...
    if (rows.length === 0 || !rows[0][0]) {
      return json({ ok: false, source: "loader", error: "TWSE CSV Ëß??Â§±Ê?" });
    }
    // 3. Filter & upsert
...
// ?Ä?Ä loadMarketPricesFinMind: FinMind TaiwanStockPrice ??market_price_bars ?Ä?Ä
// 2026-08-13: backup source for ?ãËÇ° OHLC (free, no token needed).
// FinMind provides trading_money + trading_turnover which Yahoo Finance Ê≤íÊ???// Per-stock API call (FinMind ‰∏çÊîØ??batch via query string), parallel with 8-slot throttle.
...
async function loadMarketPricesFinMind(request) {
  ...
  if (u.searchParams.get("code")) {
    ...
    // Ê≤íÊ?ÂÆ?????watchlist + etf_watchlist
  ...
  if (codes.length === 0) {
    return json({ ok: true, source: "stub", count: 0, message: "Ê≤íÊ? code ?ØË??? });
  }
  // Ë®àÁ? start_date
  ...
  // Âπ≥Ë? 8 ??in-flight (FinMind Ê≤íÂ???limit ‰ΩÜ‰?ÂÆà‰?Èª?
  const PARALLEL = 8;
  const results = [];
  const errors = [];
  for (let i = 0; i < codes.length; i += PARALLEL) {
    ...
    const batchRes = await Promise.all(batch.map(async (code) => {
      ...
      try {
        ...
        // ??watchlist Ë∑?etf_watchlist Â∞çÊ???asset_type
        const [wl, ew] = await Promise.all([
...
const TWSE_INDUSTRY_MAP = {
  "2330": "?äÂ?È´îÊ•≠",     "2454": "?äÂ?È´îÊ•≠",   "2303": "?äÂ?È´îÊ•≠",   "2308": "?äÂ?È´îÊ•≠",
  "2379": "?äÂ?È´îÊ•≠",     "3711": "?äÂ?È´îÊ•≠",   "3034": "?äÂ?È´îÊ•≠",   "6669": "?äÂ?È´îÊ•≠",
  "3231": "?ªËÖ¶?äÈÄ±È?Ë®≠Â?Ê•?,"2357": "?ªËÖ¶?äÈÄ±È?Ë®≠Â?Ê•?,"2382": "?ªËÖ¶?äÈÄ±È?Ë®≠Â?Ê•?,
  "0050": "ETF",          "0051": "ETF",        "0052": "ETF",        "0056": "ETF",  "00878": "ETF",
  "2881": "?ëË?‰øùÈö™Ê•?,   "2882": "?ëË?‰øùÈö™Ê•?, "2884": "?ëË?‰øùÈö™Ê•?, "2885": "?ëË?‰øùÈö™Ê•?,
  "2886": "?ëË?‰øùÈö™Ê•?,   "2887": "?ëË?‰øùÈö™Ê•?, "2891": "?ëË?‰øùÈö™Ê•?, "2892": "?ëË?‰øùÈö™Ê•?,
  "1301": "Â°ëË?Â∑•Ê•≠",     "1303": "Â°ëË?Â∑•Ê•≠",   "1326": "Â°ëË?Â∑•Ê•≠",   "6505": "Â°ëË?Â∑•Ê•≠",
  "2002": "?ºÈêµÂ∑•Ê•≠",     "2207": "Ê±ΩË?Â∑•Ê•≠",   "3008": "?âÈõªÊ•?,     "1101": "Ê∞¥Ê≥•Â∑•Ê•≠",
  "2317": "?∂‰??ªÂ?Ê•?,
  "1216": "È£üÂ?Â∑•Ê•≠",
};
...
// ?Ä?Ä loadAllCombined: combined loader for Vercel Hobby (1 cron slot) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
async function loadAllCombined(request) {
  ...
  // 5. sectors (Á°¨Á∑® TWSE industry mapping)
...
// ?Ä?Ä loadMarketPricesBackfill: Yahoo Finance ?ãËÇ°?•Ê≠∑????market_price_bars ?Ä
...
// ?Ä?Ä MOPS revenue loader (?àÁ???from ?¨È?Ë≥áË?ËßÄÊ∏¨Á?) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
// Flow:
...
async function loadRevenueForMonth(yearRoc, month, typek = "sii") {
  ...
  // ?∫Ë°®?•Ê?,Ë≥áÊ?Âπ¥Ê?,?¨Âè∏‰ª??,?¨Âè∏?çÁ®±,?¢Ê•≠???üÊ•≠?∂ÂÖ•-?∂Ê??üÊî∂,?üÊ•≠?∂ÂÖ•-‰∏äÊ??üÊî∂,?üÊ•≠?∂ÂÖ•-?ªÂπ¥?∂Ê??üÊî∂,?üÊ•≠?∂ÂÖ•-‰∏äÊ?ÊØîË?Â¢ûÊ?(%),?üÊ•≠?∂ÂÖ•-?ªÂπ¥?åÊ?Â¢ûÊ?(%),Á¥ØË??üÊ•≠?∂ÂÖ•-?∂Ê?Á¥ØË??üÊî∂,Á¥ØË??üÊ•≠?∂ÂÖ•-?ªÂπ¥Á¥ØË??üÊî∂,Á¥ØË??üÊ•≠?∂ÂÖ•-?çÊ?ÊØîË?Â¢ûÊ?(%),?ôË®ª
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    seen.add(key);
    symbols.push(sym);
    // cols: 5=?∂Ê??üÊî∂, 8=MoM%, 9=YoY%, 10=YTD Á¥ØË??üÊî∂, 12=YTD YoY%
...
// ?Ä?Ä TAIFEX futures loader (Â§ßÂè∞/Â∞èÂè∞/?ªÂ?/?ëË?/ÂæÆÂ?) ?Ä?Ä
...
// CSV: Big5 encoded. Columns: ‰∫§Ê??•Ê?,Â•ëÁ?,?∞Ê??à‰ªΩ(?±Âà•),?ãÁõ§???ÄÈ´òÂÉπ,?Ä‰ΩéÂÉπ,?∂Áõ§??Êº≤Ë???Êº≤Ë?%,?ê‰∫§??ÁµêÁ????™Ê??∑Â?Á¥ÑÊï∏,...
// Map: contract=Â•ëÁ? (TX), maturity=?∞Ê??à‰ªΩ, all others as-is.
const TAIFEX_FUTURE_CONTRACTS = [
  { id: "TX", name: "?∫ËÇ°?üË≤®" },
  { id: "MTX", name: "Â∞èÂ??∫ËÇ°?üË≤®" },
  { id: "TE", name: "?ªÂ??üË≤®" },
  { id: "TF", name: "?ëË??üË≤®" },
  { id: "ZEF", name: "ÂæÆÂ??∫Ê??üË≤®" },
];
...
async function loadFuturesForDate(dateYmd) {
  ...
  for (const c of TAIFEX_FUTURE_CONTRACTS) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Filter to "‰∏Ä?? (regular session) only ??the CSV also has ?§Â? (after-hours) and Â§úÁõ§
...
// ?Ä?Ä ai_capex loader: SEC EDGAR companyconcept API ??ai_capex table ?Ä?Ä?Ä?Ä?Ä
...
// ?Ä?Ä etf_holdings loader: per-issuer scraping (PROBE ONLY) + manual seed ?Ä?Ä
...
async function seedEtfHoldings(request) {
  ...
  if (!etfCode || !asOf || holdings.length === 0) {
    return json({ error: "Áº∫Â?ÂøÖË?Ê¨Ñ‰?Ôºöetf_code, as_of_date (YYYY-MM-DD), holdings[]" }, { status: 400 });
  }
...
// ?Ä?Ä seedBigHolders: manual POST endpoint for big_holders ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
// Body: { password, holders: [{ symbol, holder_type, holder_name, shares, pct, as_of_date }] }
// Use case: real Â§ßËÇ°???¨Â? from MOPS / broker feeds / manual entry.
async function seedBigHolders(request) {
  ...
  if (holders.length === 0) {
    return json({ error: "Áº∫Â?ÂøÖË?Ê¨Ñ‰?Ôºöholders[] (each with symbol, holder_type, holder_name, shares, pct, as_of_date)" }, { status: 400 });
  }
...
// ?Ä?Ä seedFinancialReports: manual POST endpoint for financial_reports ?Ä?Ä
...
async function seedFinancialReports(request) {
  ...
  if (reports.length === 0) {
    return json({ error: "Áº∫Â?ÂøÖË?Ê¨Ñ‰?Ôºöreports[] (each with symbol, period, revenue, gross_profit, operating_income, net_income, eps)" }, { status: 400 });
  }
  let inserted = 0, updated = 0, skipped = 0;
...
// ?Ä?Ä placeholders (return helpful shape, not pure stub) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
// 2026-08-26: Ë£?watchlist ‰∏≠Áº∫ K Á∑öÁ??°„ÄÇÈ?ËºØË? seed-add-stocks.mjs ?∏Â???//   body: { password: string, codes?: string[], days?: number }
//   ‰∏çÂÇ≥ codes ???™Â???watchlist ?æÁº∫Ë≥áÊ??ÑËÇ°
//   ÂØÜÁ¢º: STOCK_OPERATOR_PASSWORD env
...
// ?Ä?Ä router ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
const TABLE = [
  ...
  // 2026-08-26: Ë£?watchlist Áº?K Á∑öÁ??°Ô??™Â??ÉÊ??áÂ? codesÔº?  ["POST", /^\/admin\/reseed-stocks\/?$/,          adminReseedStocks],
