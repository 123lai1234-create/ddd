...
(function () {
  ...
  async function boot() {
    ...
    // 3. 抓 CWA 資料(純前端 <script> 注入,免 server proxy)
    let cwaData = null;
    try {
      if (!window.CWALoader) {
        throw new Error('CWALoader not loaded — make sure cwa-loader.js is included before diving-charts.js');
      }
      const r = await window.CWALoader.load('46694A');
      if (r.ok && r.data) {
        cwaData = r.data;
        console.log('[diving] CWA data loaded:', cwaData.snapshot);
      } else {
        console.warn('[diving] CWA load failed:', r.error);
      }
    } catch (e) {
      console.warn('[diving] CWA fetch threw:', e.message);
    }
