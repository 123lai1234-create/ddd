(() => {
    ...
    const setStatus = (msg, state) => { const el = $('videoStatus'); if(!el) return; el.textContent = msg; el.dataset.state = state||'info'; };
    const setGenerating = (active) => { const btn = $('videoGenBtn'); if(!btn) return; btn.disabled = active; btn.textContent = active ? '生成中...' : '生成影片'; const sp = $('videoSpinner'); if(sp) sp.hidden = !active; };
    const showVideo = (url) => { const v = $('videoResult'); const p = $('videoPlaceholder'); if(!v) return; v.src = url; v.hidden = false; if(p) p.hidden = true; };
    const stopPolling = () => { if(pollTimer){ clearInterval(pollTimer); pollTimer = null; } };
    const pollStatus = async (api, rid) => {
        try {
            const r = await fetch(api+'/api/video/status/'+rid);
            if(!r.ok){ const e=await r.json().catch(()=>({})); stopPolling(); setGenerating(false); setStatus('錯誤：'+(e.detail||r.statusText),'error'); return; }
    ...
    const generate = async (api) => {
        ...
        try {
            const r = await fetch(api+'/api/video/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt,resolution,duration})});
            const d = await r.json();
    ...
    const init = async () => {
        ...
        document.querySelectorAll('[data-example-prompt]').forEach(b=>b.addEventListener('click',()=>{ const el=$('videoPrompt'); if(el){el.value=b.dataset.examplePrompt;el.focus();} }));
        document.querySelectorAll('.faq-header').forEach(h=>h.addEventListener('click',()=>{ const b=h.nextElementSibling; if(!b) return; b.hidden=!b.hidden; const a=h.querySelector('.faq-arrow'); if(a) a.textContent=b.hidden?'▼':'▲'; }));
    };
