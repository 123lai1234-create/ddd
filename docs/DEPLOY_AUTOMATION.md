...
- 靜態站 / 作品集：`donttalk`
- API 服務：`donttalk-api`
- 前端 runtime 目前以 `donttalk` / `donttalk-api` 為唯一主命名。

## 目前 workflow 綁定的既有平台資源
...
- `render.yaml`、`fly.toml`、`wrangler.toml` 已對齊為 `donttalk` / `donttalk-api` 命名。
- 前端的 API 自動偵測目前也以 `donttalk` / `donttalk-api` 為預設來源。
- `vercel.json` 與 `netlify.toml` 目前仍指向可用的 Railway public domain fallback。
