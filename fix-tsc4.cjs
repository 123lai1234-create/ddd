...
// The cleanest TypeScript-friendly fix: cast `r` to `any` inside the mapper,
// so the resulting object literal has `any` for each field and is a homogeneous shape.
const fixes = [
  ...
  ["const okCount = results.map(r => ({ok: r.ok, count: (r && r.count) || 0})).filter(x => x.ok).reduce((s, x) => s + x.count, 0);",
   "const okCount = results.map(r => ({ok: r.ok, count: (r && r.count) || 0})).filter(x => x.ok).reduce((s, x) => s + (x.count || 0), 0);"],
  ...
  ["const inserted = results.map(r => ({ok: r.ok, inserted: (r && r.inserted) || 0})).filter(x => x.ok).reduce((s, x) => s + x.inserted, 0);",
   "const inserted = results.map(r => ({ok: r.ok, inserted: (r && r.inserted) || 0})).filter(x => x.ok).reduce((s, x) => s + (x.inserted || 0), 0);"],
];
let changed = 0;
