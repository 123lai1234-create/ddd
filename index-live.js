let _portfolioApiBase = '';
        ...
        async function portfolioApiJson(path) {
            ...
            if (!response.ok) {
                ...
                throw new Error(data?.detail || `HTTP ${
                            response.status
                        }
        ...
        function renderPortfolioSequenceList(containerId, records, sequenceType) {
            ...
            container.innerHTML = records.map((record) => {
                ...
                const tertiary = sequenceType === 'gene'
                    ...
                    ? `${
                            Number(record.gcContent || 0).toFixed(1)
                        }

                        % GC` : portfolioEscapeHtml(record.queryTerm || 'protein search');
                const link = record.recordUrl ? `<a class="live-seq-link" href="${portfolioEscapeHtml(record.recordUrl)}" target="_blank" rel="noreferrer" >來源</a>` : '<span></span>';

                return ` <article class="live-seq-item" > <div class="live-seq-item-head" > <div> <div class="live-seq-item-title" >${
                            portfolioEscapeHtml(record.displayName)
                        }

                        </div> <div class="live-seq-item-sub" >${
                            portfolioEscapeHtml(record.organism)
                        }

                        </div> </div> <div class="live-seq-pill" >${
                            sequenceType.toUpperCase()
                        }

                        </div> </div> <div class="live-seq-preview" >${
                            portfolioEscapeHtml(portfolioSequencePreview(record.sequence))
                        }

                        </div> <div class="live-seq-item-meta" > <span>${
                            Number(record.sequenceLength || 0).toLocaleString()
                        }
                        ...
                        ${
                            ...
                            portfolioEscapeHtml(record.sourceName || '')
                        }
        ...
        function renderPortfolioKnowledgeList(containerId, records, recordType) {
            ...
            container.innerHTML = records.map((record) => {
                ...
                const sourceLink = record.recordUrl ? `<a class="live-seq-link" href="${portfolioEscapeHtml(record.recordUrl)}" target="_blank" rel="noreferrer" >來源</a>` : '<span></span>';

                return ` <article class="live-seq-item" > <div class="live-seq-item-head" > <div> <div class="live-seq-item-title" >${
                            portfolioEscapeHtml(record.title)
                        }
                        ...
                        · ${
                            portfolioEscapeHtml(record.sourceId || '-')
                        }
                        ...
                        </div> </div> <div class="live-knowledge-preview" >${
                            portfolioEscapeHtml(preview || 'No summary available.')
                        }

                        </div> <div class="live-seq-item-meta" > <span>${
                            portfolioEscapeHtml(record.organism || record.publishedAt || '-')
                        }

                        </span> <span>${
                            portfolioEscapeHtml(record.queryTerm || '-')
                        }
        ...
        function renderPortfolioRagPreview(documents) {
            ...
            container.innerHTML = documents.map((document) => ` <article class="live-rag-item" > <div class="live-seq-item-head" > <div> <div class="live-seq-item-title" >${
                        portfolioEscapeHtml(document.title || document.documentId)
                    }

                    </div> <div class="live-seq-item-sub" >${
                        portfolioEscapeHtml(document.chunkId || '-')
                    }

                    </div> </div> <div class="live-seq-pill" >${
                        portfolioEscapeHtml(document.embeddingHint || 'rag')
                    }

                    </div> </div> <div class="live-rag-text" >${
                        portfolioEscapeHtml(String(document.text || '').trim() || 'No text available.')
                    }

                    </div> <div class="live-rag-meta" > <span>${
                        portfolioEscapeHtml(document.metadata?.sourceName || '-')
                    }

                    </span> <span>${
                        portfolioEscapeHtml(document.metadata?.sourceId || '-')
                    }

                    </span> <span>${
                        portfolioEscapeHtml(document.metadata?.queryTerm || '-')
                    }
        ...
        async function loadPortfolioSequenceFeed() {
            ...
            try {
                const [summary, proteinPayload, genePayload] = await Promise.all([portfolioApiJson('/api/sequences/summary'),
                ...
                setPortfolioSequenceStatus(`序列資料庫讀取失敗：${
                            error.message
                        }
        ...
        async function loadPortfolioKnowledgeFeed() {
            ...
            try {
                const [summary, annotationPayload, literaturePayload, ragPayload] = await Promise.all([portfolioApiJson('/api/knowledge/summary'),
                ...
                setPortfolioKnowledgeStatus(`知識資料庫讀取失敗：${
                            error.message
                        }
