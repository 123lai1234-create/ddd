...
# ──────────────────────────────────────────────────────────────────────
# Web scraper demo — URL → title/meta/og/links/text preview
# Security: blocks private IPs, limits response size, short timeout
# ──────────────────────────────────────────────────────────────────────
...
def _is_safe_url(url: str) -> tuple[bool, str]:
    ...
    for fam, _, _, _, sa in addrs:
        ip = sa[0]
        try:
            ipobj = _ipaddr.ip_address(ip)
        except ValueError:
            continue
        if ipobj.is_private or ipobj.is_loopback or ipobj.is_link_local or ipobj.is_reserved or ipobj.is_multicast:
            return False, f"blocked host resolves to {ip}"
    return True, ""


_TAG_RE = _re.compile(r"<[^>]+>")
_SCRIPT_RE = _re.compile(r"<(script|style)[^>]*>.*?</\1>", _re.I | _re.S)
_WS_RE = _re.compile(r"\s+")


def _extract(html: str, base_url: str) -> dict[str, Any]:
    def match(pat: str, src: str = html, group: int = 1) -> str:
        m = _re.search(pat, src, _re.I | _re.S)
        return _unescape(m.group(group).strip()) if m else ""

    title = match(r"<title[^>]*>(.*?)</title>")
    description = match(r'<meta\s+name=["\']description["\']\s+content=["\']([^"\']+)["\']')
    og_title = match(r'<meta\s+property=["\']og:title["\']\s+content=["\']([^"\']+)["\']')
    og_image = match(r'<meta\s+property=["\']og:image["\']\s+content=["\']([^"\']+)["\']')

    h1s = [_unescape(_TAG_RE.sub("", m).strip())
           for m in _re.findall(r"<h1[^>]*>(.*?)</h1>", html, _re.I | _re.S)][:5]
    h1s = [h for h in h1s if h]

    links = []
    seen = set()
    for m in _re.finditer(r'<a\s+[^>]*href=["\']([^"\']+)["\'][^>]*>(.*?)</a>', html, _re.I | _re.S):
        href = m.group(1).strip()
        text = _WS_RE.sub(" ", _unescape(_TAG_RE.sub("", m.group(2))).strip())
    ...
    text = _WS_RE.sub(" ", _TAG_RE.sub(" ", clean)).strip()
    preview = text[:800]
...
def scrape_url(payload: ScrapeRequest, request: Request) -> dict[str, Any]:
    ...
    try:
        with httpx.Client(
            ...
            headers={"User-Agent": "JTLai-Portfolio-Scraper/1.0 (+https://donttalk.vercel.app)"},
            max_redirects=5,
        ) as client:
            resp = client.get(url)
            resp.raise_for_status()
            if len(resp.content) > 2_000_000:
                raise HTTPException(status_code=413, detail="response too large (>2MB)")
        ...
        raise HTTPException(status_code=502, detail=f"upstream HTTP {exc.response.status_code}")
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"fetch failed: {exc.__class__.__name__}")

    data = _extract(html, str(resp.url))
    data["url"] = str(resp.url)
    data["status_code"] = resp.status_code
    data["size_bytes"] = len(resp.content)
    data["fetched_at"] = int(_time.time())
    return data
